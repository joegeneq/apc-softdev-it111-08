...
class DPCalendarModelProfile extends JModelList
{
	...
	protected function getListQuery ()
	{
		...
		$query->from($db->quoteName('#__dpcalendar_caldav_calendars') . ' c');
	...
	protected function populateState ($ordering = null, $direction = null)
	{
		...
		$orderCol = JRequest::getCmd('filter_order', 'c.displayname');
	...
	public function getEvents ()
	{
		...
		$model->getState();
		$model->setState('filter.parentIds', array(
				'root'
		));
		$ids = array();
		...
		if ($action == 'add')
		{
			$query = 'insert into #__dpcalendar_caldav_groupmembers (member_id, principal_id)
					select ' . (int) $id . ' as member_id, id as principal_id from #__dpcalendar_caldav_principals
							where uri=' .
					 $db->quote('principals/' . JFactory::getUser()->username . '/calendar-proxy-' . $type);
		}
		else
		{
