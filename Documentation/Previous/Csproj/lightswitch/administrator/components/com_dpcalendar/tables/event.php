...
JLoader::import('components.com_dpcalendar.libraries.ical.iCalcreator', JPATH_ADMINISTRATOR);
...
class DPCalendarTableEvent extends JTable
{
	...
	public function store ($updateNulls = false)
	{
		...
		{
			$appendix = str_replace(array(
					':',
					' ',
					'-'
			), '', $this->start_date);
			if ($table->load(array(
					'alias' => $this->alias . '_' . $appendix,
					'catid' => $this->catid
			)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_DPCALENDAR_ERROR_UNIQUE_ALIAS') . ': ' . $this->alias);
				return false;
			}

			$this->alias = $this->alias . '_' . $appendix;
		}
		...
		if ($this->id > 0)
		{
			...
			$hardReset = $this->all_day != $table->all_day || $this->start_date != $table->start_date || $this->end_date != $table->end_date ||
		...
		$cal->expand(new DateTime('1970-01-01'), new DateTime('2038-01-01'));
		...
		foreach ($cal->VEVENT as $vevent)
		{
			...
			else
			{
				...
				// Fields to update.
				$files = array(
						$this->_db->qn('alias') . ' = ' . $this->_db->q($table->alias . '_' . $startDate->format('U')),
						...
						$this->_db->qn('xreference') . ' = ' . $this->_db->q($this->xreference),
	...
	{
		if (JFilterInput::checkAttribute(array(
				'start_date',
				$this->start_date
		)))
		{
		...
		if (trim($this->title) == '')
		{
			$this->setError(JText::_('COM_DPCALENDAR_ERR_TABLES_TITLE'));
			return false;
		}

		// Check for existing name
		$query = 'SELECT id, original_id FROM #__dpcalendar_events WHERE alias = ' . $this->_db->Quote($this->alias) . ' AND catid = ' .
				 (int) $this->catid;
		$this->_db->setQuery($query);
		...
		if ($xid && $xid->id != intval($this->id) && $xid->original_id == $this->original_id)
		{
			...
			$query = 'SELECT id, original_id FROM #__dpcalendar_events WHERE alias = ' . $this->_db->Quote($this->alias . '_' . $appendix) .
			...
			if ($xid && $xid->id != intval($this->id) && $xid->original_id == $this->original_id)
			{
				$this->setError(JText::_('COM_DPCALENDAR_ERR_TABLES_NAME') . ' ' . $xid->alias);
				return false;
			}
	...
	public function delete ($pk = null)
	{
		...
		if ($success && $pk > 0)
		{
			...
			$this->_db->execute();
			$this->_db->setQuery('delete from #__dpcalendar_attendees where event_id = ' . (int) $pk);
			$this->_db->execute();
		}
		return $success;
	}
	...
	public function attend ($increment = true, $pk = null)
	{
		if ($pk == null)
		{
			$pk = $this->id;
