...
class DPCalendarHelper
{
	...
	public static function renderEvents (array $events = null, $output, $params = null, $eventParams = array())
	{
		...
		foreach ($events as $event)
		{
			...
			$variables['month'] = strtoupper(self::getDate($event->start_date, $event->all_day)->format('M', true));
	...
	public static function getEventRoute ($id, $calId, $full = false)
	{
		$needles = array(
				'event' => array(
						(int) $id
	...
	public static function loadLibrary ($libraries = array('jquery' => true))
	{
		...
		{
			$document->addScript(
					(JBrowser::getInstance()->isSSLConnection() ? "https" : "http") . '://maps.googleapis.com/maps/api/js?sensor=true&language=' .
							 self::getGoogleLanguage());
	...
	public static function where ()
	{
		...
		foreach (debug_backtrace() as $k => $v)
		{
			...
			$trace .= '#' . ($k - $ignore) . ' ' . $v['file'] . '(' . $v['line'] . '): ' . (isset($v['class']) ? $v['class'] . '->' : '') .
