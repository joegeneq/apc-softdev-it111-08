dpjQuery(document).ready(function(){
	dpjQuery('#scheduling-options, #jform_scheduling_monthly_options').bind('change', function(e) {
		changeVisiblity();
		updateRuleFromForm();
	});
	dpjQuery('#jform_scheduling, #jform_scheduling_end_date, #jform_scheduling_interval, #jform_scheduling_repeat_count').bind('change', function() {
		updateRuleFromForm();
	});
	...
	dpjQuery.each(dpjQuery('#jform_rrule').val().split(';'), function(){
		...
		if(parts.length > 1){
			switch(parts[0]){
				case 'FREQ':
					frequency = parts[1];
					dpjQuery('#jform_scheduling input[value="'+parts[1]+'"]').attr('checked', 'checked');
					break;
				case 'BYDAY':
...
function updateLocationFrame(){
	var dpcalendarMap = new google.maps.Map(document.getElementById('event-location-frame'), {zoom: 4, mapTypeId: google.maps.MapTypeId.ROADMAP, center: new google.maps.LatLng(geoip_latitude(), geoip_longitude())});
