...
abstract class DPCalendarPlugin extends JPlugin
{
	...
	{
		$content = $this->getContent($calendarId, $startDate, $endDate, $options);
		if (empty($content))
		{
			return array();
		}
		...
		usort($data,
				...
				{
					return strcmp($event1->DTSTART->getDateTime()->format('U'), $event2->DTSTART->getDateTime()->format('U'));
				});
	...
	public function onEventsFetch ($calendarId, JDate $startDate = null, JDate $endDate = null, JRegistry $options = null)
	{
		...
		if ($startDate)
		{
			...
			$startDate->setTime($startDate->format('H'), $startDate->format('m'), 0);
		}
	...
	{
		$allDay = $event->DTSTART->getDateType() == Sabre\VObject\Property\DateTime::DATE;
		...
		if (! empty($location))
		{
			...
			{
				$locations[] = DPCalendarHelperLocation::get($location);
			}
		}
	...
	private function icalDecode ($text)
	{
		$newText = str_replace('\n', '<br/>', $text);
