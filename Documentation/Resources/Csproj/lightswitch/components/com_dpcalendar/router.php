...
function dPCalendarBuildRoute (&$query)
{
	$segments = array();
	...
	if (isset($query['view']))
	{
		...
		// We need to keep the view for forms since they never have their own
		// menu item
		if ($view != 'form' && $view != 'davcalendar' && $view != 'attendee' && $view != 'pay' && $view != 'message' && $view != 'callback')
		{
			unset($query['view']);
		}
	}
...
function dPCalendarParseRoute ($segments)
{
	$vars = array();
