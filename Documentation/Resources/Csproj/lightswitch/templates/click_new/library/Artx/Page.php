...
class ArtxPage
{
    ...
    public function positions($positions, $style)
    {
        ...
        if (count($cells) == 0)
            return '';
        $result = '<div class="za-content-layout">';
        $result .= '<div class="za-content-layout-row">';
        foreach ($cells as $name => $width)
            $result .='<div class="za-layout-cell' . ('za-block' == $style ? ' za-layout-sidebar-bg' : '')
     ...
     *
     * The componentWrapper method gets the content of the 'component' buffer and searches for the '<div class="za-post">' string in it.
     * Then it wraps the content of the buffer with the za-post.
     */
    public function componentWrapper()
    {
        ...
        if (false === strpos($content, '<div class="za-post'))
        ...
        CKEDITOR.on('instanceReady', function (evt) {
            ...
            if (includesTemplateStyle || includesEditorStyle) {
                ...
                if (includesTemplateStyle)
                    jQuery('body', ifrdoc).attr('id', 'za-main').addClass('za-postcontent');
            }
        });
    }
});
