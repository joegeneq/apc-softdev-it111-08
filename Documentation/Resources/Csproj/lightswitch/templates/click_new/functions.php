...
if (!defined('_ARTX_FUNCTIONS')) {
    ...
    /**
     * Decorates an article or block with the za-post style.
     *
    ...
    function artxPost($data)
    {
        ...
        $classes = isset($data['classes']) && strlen($data['classes']) ? $data['classes'] : '';
                    artxFragmentBegin("<article class=\"za-post" . $classes . "\">");
            artxFragmentBegin("<h2 class=\"za-postheader\"><span class=\"za-postheadericon\">");
    ...
    function artxBlock($caption, $content, $classes = '')
    {
        ...
        if (!$hasCaption && !$hasContent)
            return '';
                    artxFragmentBegin("<div class=\"za-block clearfix" . $classes . "\">");
            artxFragmentBegin("<div class=\"za-blockheader\"><h3 class=\"t\">");
            artxFragmentContent($caption);
            artxFragmentEnd("</h3></div>");
            artxFragmentBegin("<div class=\"za-blockcontent\">");
    ...
    function artxVMenuBlock($caption, $content, $classes = '')
{
    ...
    if (!$hasCaption && !$hasContent)
        return '';
                artxFragmentBegin("<div class=\"za-vmenublock clearfix" . $classes . "\">");
            artxFragmentBegin("<div class=\"za-vmenublockheader\"><h3 class=\"t\">");
            artxFragmentContent($caption);
            artxFragmentEnd("</h3></div>");
            artxFragmentBegin("<div class=\"za-vmenublockcontent\">");
            artxFragmentContent(artxPostprocessBlockContent($content));
            artxFragmentEnd("</div>");
     ...
     *  Select inputs with class != 'za-button'
     *   artxFindTags($html, array('input' => array('class' => '!za-button')))
     *  Select inputs with non-empty class
     *   artxFindTags($html, array('input' => array('class' => '!')))
     *  Select inputs with class != 'za-button' and non-empty class:
     *   artxFindTags($html, array('input' => array('class' => array('!za-button', '!'))))
     *  Select inputs with class = button but != 'za-button'
     *   artxFindTags($html, array('input' => array('class' => array('button', '!za-button'))))
     *
    ...
    function artxReplaceButtons($content, $filter = array('input' => array('class' => 'button')))
    {
        ...
        foreach (artxFindTags($content, $filter) as $tag) {
            if (is_string($tag))
                ...
                $tag['attributes']['class']['value'][] = 'za-button';
    ...
    function artxLinkButton($data = array())
    {
        return '<a class="' . (isset($data['classes']) && isset($data['classes']['a']) ? $data['classes']['a'] . ' ' : '')
            . 'za-button" href="' . $data['link'] . '">' . $data['content'] . '</a>';
    }
    ...
    function artxPostprocessContent($content)
    {
        ...
        $content = artxReplaceButtons($content, array('input' => array('class' => array('button', '!za-button')),
            'button' => array('class' => array('button', '!za-button')), 'button' => array('class' => array('btn', '!za-button'))));
        return $content;
    }
