...
class ModSimpleFileUploaderHelperv13{	
	...
	function getUploadForm(
			...
			$upload_fileextensions = $params->get( 'upload_fileextensions', '.gif .jpg .jpeg .png' );
			...
			for ($cnt = 0; $cnt<$fileCnt; $cnt++) {
				...
				if ((strlen($_FILES["uploadedfile$mid"]["name"][$cnt]) > 0) && ($upload_users === "true")) {
					...
					if ($filetypeok) {
						...
						// Check extension
						$extensions = explode(' ', strtolower($upload_fileextensions)); //array('.png', '.jpg', '.zip', '.pdf'); // specify the allowed extensions here
						$ext = strtolower(strrchr($_FILES["uploadedfile$mid"]["name"][$cnt],'.'));
					...
					if (($filetypeok) && ($_FILES["uploadedfile$mid"]["size"][$cnt] < $upload_maxsize)) {
						...
						if ($written == 0) {
							...
							if ($upload_filetypes === "*") $upload_filetypes = $upload_fileextensions;
